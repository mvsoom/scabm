...
begin # ABM parameters
	N₀ = 40    # Expected number of agents
...
begin
	function weigh!(ensemble, λ)
		@transform!(ensemble, :logweight = -λ*(:C))
	end
...
begin
	...
	function diagram!(p, ensemble; c = 10, n = 100, z = 3, color = :black)
		...
		for (k, sᵍ) in pairs(s)
			...
			sᵍ = sᵍ[keep, :]
			@df sᵍ plot!(p, :μᶜ, :μˢ; linewidth=3, linecolor = color)
			annotate!(p, μ, 1., text(k.graphtype, "Arial", 6))
		end
	end
end
...
p = let
	...
	diagram!(p[2], ensemble)

	p
end

# ╔═╡ 6dc81183-8a8c-4aa2-bc73-3dd68765c3b3
let
	savefig(p, "main.png")
end
